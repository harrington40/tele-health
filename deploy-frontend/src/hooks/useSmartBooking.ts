...
export const useSmartBooking = () => {
  ...
  const runSmartBooking = useCallback(async (serviceId: number) => {
    ...
    try {
      ...
      const results = await algorithm.findOptimalDoctor(mockDoctors, state.preferences);
  ...
  // Real-time slot availability updates (disabled for now - result now contains doctor, not slots)
  // useEffect(() => {
