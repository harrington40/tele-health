...
const DoctorsPage: React.FC = () => {
  ...
  useEffect(() => {
    async function fetchDoctors() {
      // Import gRPC client
      const { getAllDoctors } = await import('../services/grpcClient');
      getAllDoctors((err: any, response: any) => {
        if (!err && response && response.doctors) {
  ...
  useEffect(() => {
    ...
    const sorted = sortDoctors(filtered, sortBy);
    const initialDoctors = sorted.slice(0, 8);

    setDisplayedDoctors(initialDoctors);
    setPage(1);
  }, [searchTerm, specialty, sortBy, doctors]);
